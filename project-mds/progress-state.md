...
- M15 (Test coverage) — IN PROGRESS. M15.1.1–.3 DONE (web jest harness + chat 10 + history 10). M15.1.4 DONE: `apps/web/app/account/page.test.tsx` (8 tests) — current-plan badge, M6.3 metered usage meter, disabled-metered "not included", boolean included/not-included rows, upgrade CTA→`POST /billing/checkout` redirect, "Manage billing"→`POST /billing/portal`, load-error state, VI locale-aware currency ("9,99 …/tháng"). (Locale-toggle persistence + sign-out aren't on the account page — they're provider/topbar mechanisms deferred to M15.1.5/.6.) Rest of M15.1.5–.6 / M15.2 (admin) / M15.3 (E2E) open.
- Tests: 1313 pass / 0 fail / 0 skip (shared 190, ui 234, db 9, ai 161, api 686, web 33). (admin has no jest suite yet.)
...
- Next tasks (priority order):
  1. **M15.1.5–.6** — web i18n tests (locale provider EN→VI switching + persistence, dictionary key completeness, locale-aware formatters) + shared hooks/lib tests (`useMediaQuery`, `useLocale`/`useT`, API client fns, firebase emulator-aware init).
  2. **M15.2** (admin jest suite — needs its own harness mirroring web) + **M15.3** (E2E expansion).
