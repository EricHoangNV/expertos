...
- M15 (Test coverage) — IN PROGRESS. M15.1.1–.4 DONE (web jest harness + chat 10 + history 10 + account 8). M15.1.5 DONE: `apps/web/src/lib/i18n/i18n.test.tsx` (9 tests) — LocaleProvider EN→VI switching + localStorage/`PATCH /me/locale` write-through, resolution order (localStorage pref > profile seed), EN/VI dictionary lockstep (key set + non-empty leaves + interpolation-placeholder parity), locale-aware currency/date wired to the active locale. Rest of M15.1.6 / M15.2 (admin) / M15.3 (E2E) open.
- Tests: 1322 pass / 0 fail / 0 skip (shared 190, ui 234, db 9, ai 161, api 686, web 42). (admin has no jest suite yet.)
...
- Next tasks (priority order):
  1. **M15.1.6** — shared hooks/lib tests (`useMediaQuery`, API client fns in chat/history/account/upload clients, `firebase.ts` emulator-aware init). (`useLocale`/`useT` already covered by M15.1.5.)
