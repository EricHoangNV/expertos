...
- Tests (unit/jest): 1563 pass / 0 fail (shared 190, ui 235, db 9, ai 195, api 735, web 102, admin 97).
...
- **Security Cycle 3 — PASS (2026-06-03)**, the one open Medium now REMEDIATED (pending re-review): draft-version edit (`IngestionService.editDraftContent`) now runs a cheap `DocumentVersionRepository.assertDraftEditable` precondition BEFORE chunk/summarize/embed (reserve-before-work); the in-tx guard in `replaceDraftChunks` is retained to close the race. LEARNINGS #34.
...
1. **Host-run the E2E suite** incl. the M15.3.7 tests via `scripts/test-e2e-{users,admin}.sh` (expect ~28 pass / 2 fixme); fix any selector drift.
