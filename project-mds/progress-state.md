...
## Next tasks
0. **Continue M17** (in-sandbox-codeable): **M17.4** retrieval score floor (`minScore?` on `RetrievalRequest`, filter after `fuseHybrid` in `pgvector.store.ts`; `RetrievalService` reads `settings.getCached().retrievalScoreFloor`) → M17.5 Admin Settings page → M17.6 OpenAI embedding provider + re-embed CLI → M17.7 cutover. M17.1–M17.3 done (`SettingsService.getCached()` is the seam M17.4 reads).
