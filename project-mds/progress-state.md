...
- Tests: 1267 pass / 0 fail / 0 skip (shared 190, ui 227, db 9, ai 161, api 680). (admin has no jest suite; web has no jest suite.)
...
- Next tasks (priority order):
  ...
  2. **M13.5 (i18n)** — locale-aware date/number/currency formatting (VI date format, VND). Note `apps/web/app/account` `formatPrice` + history `when()` still use system locale.
  3. **M13.7 (admin)** — polish & ds.css conformance audit (`.voice-bar` gated on M13.5 admin).
  4. **M13.5 (admin)** — Voice profile page: blocked on a schema decision; flag to PM.
  5. **M11.1 fixme legs** / **NT human gates** — await external surfaces / PM sign-off.
