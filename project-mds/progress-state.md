...
- M15 (Test coverage) — IN PROGRESS. M15.1.1 DONE (web jest harness). M15.1.2 DONE (`apps/web/app/chat/page.test.tsx`, 10 tests). M15.1.3 DONE: `apps/web/app/history/page.test.tsx` (10 tests) — signed-out/empty states, recent-conversation list, full-text search (snippet + no-match), open→transcript replay with cited answer, save-from-transcript, inline rename, saved-answers panel load+remove, list-load error. Rest of M15.1.4–.6 / M15.2 (admin) / M15.3 (E2E) open.
- Tests: 1305 pass / 0 fail / 0 skip (shared 190, ui 234, db 9, ai 161, api 686, web 25). (admin has no jest suite yet.)
...
- Next tasks (priority order):
  1. **M15.1.4** — web account page tests (plan display + usage meter, locale toggle persistence, sign-out flow) on the harness.
  2. **M15.1.5–.6** — web i18n + hooks/lib tests, then **M15.2** (admin jest suite — needs its own harness mirroring web) + **M15.3** (E2E expansion).
