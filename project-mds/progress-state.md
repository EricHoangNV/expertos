...
- Tests: 1097 pass / 0 fail / 0 skip (shared 179, ui 89, db 9, ai 161, api 659)
...
- Next tasks (priority order):
  1. **M12 (Frontend UI Overhaul)** — M12.1 shell + M12.2 sidebar + M12.3 topbar + M12.4.1 user bubble + M12.4.2 assistant message done. Next: M12.4.3 answer prose + inline citations (AnswerView already used inside `ChatAssistantMessage` body — refine if needed), M12.4.4 horizontal action bar (refactor AnswerFeedback + SaveAnswer + View-sources toggle), M12.4.5 consultation recommendation card restyle, M12.4.6 insufficient/high-stakes/degraded card restyle, M12.5 sources rail, M12.6 input bar, M12.7 Tweaks panel. Read `requirements/ui-reference-spec.md` first. Avatar helpers (`avatarInitials`/`avatarTone` + `.avatar.tone-*`) reusable.
