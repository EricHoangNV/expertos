...
- Tests: 1137 pass / 0 fail / 0 skip (shared 179, ui 129, db 9, ai 161, api 659)
...
- Next tasks (priority order):
  1. **M12 (Frontend UI Overhaul)** — M12.1–M12.4 + M12.5.1/M12.5.2 done. Next: M12.5.3 source cards (lift the `AnswerView` `.source` markup — decoupled from `AnswerProse`, toggled by `sourcesOpen` — into the rail `children`; numbered + match% + crimson/blue doc icon + version badge + `.source-prov` + `.source-quote`), M12.5.4 drawer fallback, M12.6 input bar, M12.7 Tweaks panel, M12.9 polish/responsive. Read `requirements/ui-reference-spec.md` first. Avatar helpers (`avatarInitials`/`avatarTone` + `.avatar.tone-*`) reusable. NOTE: rebuild `packages/ui` (`tsc -p tsconfig.build.json`) after changing it — apps consume `dist/`.
