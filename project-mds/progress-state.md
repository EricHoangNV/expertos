...
- Tests: 1104 pass / 0 fail / 0 skip (shared 179, ui 96, db 9, ai 161, api 659)
...
- Next tasks (priority order):
  1. **M12 (Frontend UI Overhaul)** — M12.1 shell + M12.2 sidebar + M12.3 topbar + M12.4.1/.2/.3 done. Next: M12.4.4 horizontal action bar (refactor AnswerFeedback + SaveAnswer + a "View sources (N)" toggle into a `.btn-ghost` row), M12.4.5 consultation recommendation card restyle, M12.4.6 insufficient/high-stakes/degraded card restyle, M12.5 sources rail (the `AnswerView` sources drawer is now decoupled from `AnswerProse` — lift it into the `.sources-rail`), M12.6 input bar, M12.7 Tweaks panel. Read `requirements/ui-reference-spec.md` first. Avatar helpers (`avatarInitials`/`avatarTone` + `.avatar.tone-*`) reusable. NOTE: rebuild `packages/ui` (`tsc -p tsconfig.build.json`) after changing it — apps consume `dist/`.
