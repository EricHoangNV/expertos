...
- M17 (runtime answer-tuning settings + real embedding provider): PLANNED, IN PROGRESS. **M17.1 + M17.2 DONE.** M17.1 = `AppSettings` Prisma model (global singleton, no RLS) + migration + `packages/shared/src/app-settings.ts`. M17.2 = `apps/api/src/settings/` `SettingsService` (clones `ConciergeConfigService`: RLS-scoped `getSettings`/`updateSettings` upsert+audit-in-tx; `getCached()` 30s TTL snapshot via global client, bust-on-update) + `app-settings.controller.ts` (`@Roles("admin")` GET/PATCH) + `SettingsModule` (exports service; imported by `ChatModule`+`RetrievalModule`+`app.module`). Next: M17.3 thread temp+model through the LLM call (`LlmCallOptions`).
- Tests (unit/jest): 1632 pass / 0 fail (shared 202, ui 235, db 9, ai 195, api 788, web 102, admin 101).
...
## Next tasks
0. **Continue M17** (in-sandbox-codeable): **M17.3** thread temp+model through the LLM call (`LlmCallOptions {temperature?,model?}` on `providers.ts`; drivers honor it; `chat.service.ts:160` reads `settings.getCached()`, passes `{temperature,model}`, logs effective model) → M17.4 retrieval score floor (`minScore?` on `RetrievalRequest`, filter after `fuseHybrid` in `pgvector.store.ts`) → M17.5 Admin Settings page → M17.6 OpenAI embedding provider + re-embed CLI → M17.7 cutover. M17.1+M17.2 done (`SettingsService.getCached()` is the seam M17.3/M17.4 read).
