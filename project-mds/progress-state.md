...
- Tests: 1075 pass / 0 fail / 0 skip (shared 179, ui 67, db 9, ai 161, api 659)
...
- Next tasks (priority order):
  1. **M12 (Frontend UI Overhaul)** — M12.1 shell + M12.2 sidebar + M12.3.1 topbar title done. Next: M12.3.2 voice `.chip` picker (move the expert `<Select>` into the `.chat-topbar-aside` slot, remove it from chat-content) + M12.3.3 user identity (avatar+name+EN/VI badge), then M12.4 messages (`.msg-user`/`.msg-assistant`, AnswerView + action bar + cards), M12.5 sources rail, M12.6 input bar, M12.7 Tweaks panel. Read `requirements/ui-reference-spec.md` first. Avatar helpers (`avatarInitials`/`avatarTone` + `.avatar.tone-*`) are reusable for M12.3/M12.4.
