...
Foundation for M17 (runtime answer-tuning settings). Purely additive — a new global-singleton config table + a new isolated shared module; no existing model, route, or consumer changed.

- **Schema (`packages/db/prisma/schema.prisma`).** New `AppSettings` model placed after `ReviewConfig`: `id`, `llmTemperature Float=0.2`, `defaultChatModel String="gpt-4o-mini"`, `retrievalScoreFloor Float=0`, `createdAt`/`updatedAt`, `@@map("app_settings")`. Mirrors `ReviewConfig` — one row, no `tenantId`, no RLS (admin-guarded at the M17.2 controller). `defaultChatModel` is a plain `String` (allowlist enforced in zod/service, not a DB enum) — consistent with how the other config tables keep leaf unions out of the DB. The embedding provider is deliberately NOT a column: switching embedders invalidates existing 1536-dim vectors, so it stays env+restart (`EMBEDDING_PROVIDER`, surfaced read-only in the M17.5 UI).
- **Migration (hand-written, DIRECTIVES #35).** `20260605130000_app_settings/migration.sql`: `CREATE TABLE app_settings (…)` + explicit `GRANT SELECT, INSERT, UPDATE, DELETE … TO app_user` (the app connects as the non-superuser RLS role; the GRANT is belt-and-suspenders over the ALTER DEFAULT PRIVILEGES). Copied the `20260601060000_review_config` shape verbatim.
...
## M17.7 — Embedding cutover: local validation + reembed CLI hardening (2026-06-05)

**What**: Dry-ran the full M17 embedding cutover against the local stack end-to-end, fixed two real defects it surfaced, and left the prod deploy/verification (the genuinely external part) for the operator. Status: **PARTIAL** — procedure validated locally; M17.7 stays `[~]` until prod is cut over + verified.

**Local validation**: Set `EMBEDDING_PROVIDER=openai` in `.env` (`OPENAI_API_KEY` already present; `.env` gitignored). Dry-run (`pnpm --filter @expertos/api reembed`) reported `Active embedding provider: text-embedding-3-small (1536 dims)`, 2676 chunks — the M17.6 gate flipped correctly (no `hashing-dev` fallback warning). Full `reembed -- --commit` rewrote **all 2676 chunks, 0 failed** at the documented default `--batch=256`. Local `chunks` table now coherent in the OpenAI vector space.

**Bug #1 — reembed default batch blew Prisma's tx timeout**: The CLI wraps a whole batch of sequential `UPDATE`s in one Prisma interactive transaction; at `--batch=256` this overran Prisma's default **5s** transaction timeout, failing every full batch with `Transaction already closed: Could not perform operation` and committing only the small trailing batch (116/2676) — a silent **partial half-cutover** leaving the table in a mixed vector space (the worst failure mode). Got unblocked immediately by re-running with `--batch=64`, then root-caused and fixed: `apps/api/src/ingestion/reembed.cli.ts` now passes `{ timeout: 30_000 }` to the write `$transaction` (`TX_TIMEOUT_MS`). Re-verified the previously-failing default 256-batch completes 2676/2676, 0 failed. Committed **`ae7e234`** and pushed to `main`; api typecheck+lint + **800 tests** green. The documented one-liner cutover command now works as written.

**Bug #2 — host-DB migration drift → admin Settings 500**: The M17.5 admin "Answer settings" page rendered INTERNAL SERVER ERROR; browser console showed `GET http://localhost:3001/admin/app-settings` → 500. Root cause: the M17.1 `20260605130000_app_settings` migration (authored in-sandbox) had **never been applied to the host DB**, so `SettingsService.getSettings → findFirst(appSettings)` hit a non-existent table. `prisma migrate status` confirmed the single unapplied migration; `prisma migrate deploy` applied it (table present; the hand-added `app_user` GRANT — full privileges — verified). Not a code defect — a **deploy-ordering requirement**: the deploy pipeline must run `prisma migrate deploy` before traffic reaches the Settings page or the answer path (`getCached` reads `app_settings` via the global client as `app_user`), or prod hits the same 500.

**Host engine note**: Build + reembed run on the darwin host with the **default (library)** Prisma engine; the `PRISMA_CLIENT_ENGINE_TYPE=binary` override is sandbox-only (LEARNINGS #25). A host `nest build` was needed first — the sandbox-built `dist/` is Linux-targeted.

**Decisions**: (1) 30s tx timeout over lowering the default batch — keeps the documented `reembed -- --commit` one-liner valid and avoids more round-trips; the embed network call is already outside the tx, so the tx only spans DB writes. (2) `.env` edit (`EMBEDDING_PROVIDER=openai`) is local-only (gitignored) — the code fix is the only committed artifact; prod env is set by the operator. (3) Documented the `prisma migrate deploy` deploy-ordering requirement in PRD-TRACKING/state rather than treating the 500 as a bug.
...
Restyled `apps/admin/app/concierge-analytics/page.tsx` to match mockup screenshot 11 (one-screen-per-loop M19 parity pass). The three previously full-width `.h3` sections — "SLA adherence", "Requests by trigger mode", "Requests by visibility" — collapse into **two side-by-side titled cards** sitting directly under the KPI stat row, using the established `.matrix-foot` 2-up auto-fit grid (reused verbatim from the M13.4.4 entitlements footer — behaviorally a generic "two titled info cards" grid, so zero new ds.css per the §M19 constraint). Card 1 = `.eyebrow` "SLA adherence" over the 4 SLA `Badge` chips (tracked/met/breached/open-overdue); card 2 = `.eyebrow` "By trigger mode & visibility" merging the trigger-mode chips **and** the visibility chips into one panel, since the mockup combines them under a single heading.

Added green `.d.up` deltas to two stat cards to match the screenshot: **Answered** gets a new `answeredRate` i18n key ("{rate} answered" = answered/totalRequests, e.g. "92.0% answered") and **SLA met** reuses the existing `slaTracked` ("Tracked: {count}") key — both `trend="up"` for the green tint. The remaining three stat cards are unchanged. `byStatus` table, `reviewerVerdicts`, and `knowledgeQuality` sections stay below the panels.

i18n (`dictionaries/concierge-analytics.ts`, EN+VI lockstep): added `answeredRate` and `byTriggerModeVisibility` (the combined card title — required because the two old section headers merge into one card for screenshot parity); pruned the now-orphaned `byTriggerMode` / `byVisibility` section-header keys (only this page referenced them). `triggerModeBadge` / `visibilityBadge` / `slaAdherence` were reused as-is.
...
## M19.4.4 — recommendation-rules design parity (screenshot 15)
Restyled `apps/admin/app/recommendation-rules/page.tsx` to match the mockup (layout/styling only, real `RecommendationRulesDto` data): (1) per-row enable raw checkbox → ds.css `.switch` pill (copied entitlement-matrix toggle markup `<span className="switch"><input/><span className="track"/></span>`, wrapper `row gap1`→`row gap2`); (2) left-cell trigger-kind info/ink `Badge` → static `<span className="chip">{rule.kind.toUpperCase()}</span>` (renders KEYWORD/THRESHOLD; followed the M19.3.3 answers `<span className="chip">` precedent for a derived non-selectable label rather than the interactive `Chip` button); (3) intro `.muted`→`.lede`. i18n (EN+VI lockstep): `eyebrow` "Consultation funnel"→"When to recommend a consultation" / "Khi nào nên đề xuất tư vấn"; no new keys (kind uppercased in-component). No API/DTO/ds.css change; `RuleEditor` logic untouched. Gates: admin typecheck + next lint clean; 127/127 jest green `--runInBand` (parallel runner OOM-flakes in-sandbox, DIRECTIVE #40); root knip + lint:css clean. Flipped M19.4.4 [x] in PRD-TRACKING; build note appended. 7 M19 tasks remain (M19.4.5/.6, M19.5.1–.5).

## M19.4.5 — reconcile design parity (screenshot 16) — 2026-06-06
Design-parity polish on the admin TidyCal reconcile page. The page already matched the mockup's structure (`.pagehead` eyebrow+h1, RUN RECONCILE card with since-Input + primary Button + result Stat row, unmatched-bookings Card feed), so this was two CSS-class swaps + a copy alignment — no structural change, no DTO/client touch.
