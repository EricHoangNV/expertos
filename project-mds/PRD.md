...
- [~] **Data-retention + deletion policy reviewed and published** — uploads by mode (temporary vs persistent), consultation transcripts, concierge review records, conversation history. **TECHNICAL ENFORCEMENT BUILT; PM approval + publication pending.** Implemented: `apps/api/src/admin/retention.service.ts` `RetentionService` is the auto-delete sweeper the policy promises (and the M5.2 upload pipeline references) — admin-only, cross-tenant under the `is_admin` RLS bypass, deleting the three side-effect-free classes [`temporary` uploads past their stamped `expires_at` (chunks cascade); conversations idle past the retention window by `updated_at` (messages/citations/feedback/saved cascade); usage logs past the window by `occurred_at`] with one immutable audit entry written in the same transaction. Windows are env-tunable (`RETENTION_CONVERSATION_DAYS` / `RETENTION_USAGE_LOG_DAYS`, defaulting to the 2-year policy values); temporary-upload expiry honours the per-row stamp, not a global constant. The sweep additionally enforces the **two value-bearing classes** that must keep their structural row: consultation transcripts (`consultation_notes`) are deleted past 1yr from the consultation date (`scheduledAt ?? createdAt`) while the parent `consultations` row — revenue/MRR — is kept; concierge review records (`review_responses`) are **anonymized in place** past 1yr (answer text + reviewer notes scrubbed to a `[redacted]` sentinel, the structural row the M10.3 analytics read kept), idempotent so re-runs are no-ops (`RETENTION_CONSULTATION_TRANSCRIPT_DAYS` / `RETENTION_CONCIERGE_DAYS`, default 365). Exposed at `GET /admin/retention/preview` (non-destructive dry run) + `POST /admin/retention/sweep` (a Cloud Scheduler job targets this to run on a cadence — no in-app cron per §"No full infra Day 1"), with an `apps/admin/app/retention` operator surface. What remains is the human gate below (PM approval + publication). **DRAFT (pending PM approval):**
