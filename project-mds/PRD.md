...
#### M19 — Mockup design-parity pass for the 25 secondary screens — PLANNED

> The approved HTML mockups — now captured as **25 numbered screenshots** in `requirements/design/screenshots/pages/NN-<name>.png` — cover every screen beyond the 10 core surfaces already built in M12/M13. All 25 routes **already exist and render with the design system** (built functionally during their feature milestones, translated in M13.2/M13.3), but most were **never pinned to a mockup**. M19 is therefore a **parity/polish pass, one screen at a time — not new features**: each page is restyled to match its screenshot's layout, hierarchy, and component choices, wired to the **real** API client + DTO it already uses. The work is deliberately sliced so **one Ralph loop = one screen (M19.x.y) = one verification against one screenshot**.
>
> **Design decisions (baked in — do not re-litigate):**
> - **Keep ExpertOS branding.** The mockups render a "J&P GLOBAL" wordmark and tagline — **ignore them**. The repo's `.logo` (`Expert` ink + `OS` crimson) and all existing copy stay. Use the screenshots for **layout / hierarchy / spacing / component choice only**, never brand text.
> - **Real data only — never ship the mockups' sample data.** The names, numbers, and emails in the screenshots are illustrative. Keep each page's existing client fn + DTO (named per task); the screenshot defines look, not data. Do **not** add a DTO field to reproduce an illustrative value.
> - **Two illustrative values have no data backing — omit them, don't fake them:** the knowledge-drafts "· N asks" count (screenshot 05) and the knowledge-draft-detail "Auto-drafted from N low-confidence asks" count + "View source queries" link (screenshot 06). `KnowledgeDraftSummaryDto` / `KnowledgeDraftDto` carry no ask-count / source-query field. Render the static label without the count; only wire a real field if a separate task adds one.
> - **Shared page recipe (already the house pattern — see `apps/admin/app/analytics/page.tsx` and `requirements/Design System.md` §7):** `.pagehead` = `.eyebrow` + `.h1` (+ `.lede` intro) with the page action / window `<Select>` right-aligned → `.row .gap1` of `Stat` cards → `.h3` section headers → `Table` / `Badge` rows / `Bar` charts. Charts are CSS bars (`Bar` / `StackedBar`), no chart lib. Status → semantic `Badge` tone, never invented colors. Metadata (refs, counts, dates, IDs) → `.mono`. Loading / empty / error states per the M12.9.4 patterns.
> - **i18n lockstep.** Every new label lands in the page's existing namespace dict in **both EN and VI** (`apps/admin/src/lib/i18n/dictionaries/<ns>.ts` or `apps/web/src/lib/i18n/dictionaries.ts`), per M13. New keys are listed per task.
> - **No new ds.css needed.** The funnel "stage attribution" bars reuse the existing `Bar` component inside a labeled `.card`; the concierge-settings radio cards reuse `.card` with a crimson active border. Don't add classes unless a task explicitly says so.
> - **Don't duplicate done/planned work.** `02-my-knowledge` is **M18.3.2**, `09-voice-profile-detail` is **M13.5**, `22-access-control` is **M14.4.3**, `24-settings` is **M17.5**. Those M19 tasks are **verify-against-screenshot + minor polish**, not rebuilds — read the referenced milestone first.
>
> **Funnel — the worked template (screenshot 19, `apps/admin/app/funnel/page.tsx`, `getFunnelAnalytics` → `FunnelAnalyticsDto`):** the current page renders the KPI `Stat` row + two rate cards + a byTrigger `Table` + byResponse/byStatus `Badge` rows. The mockup adds the centerpiece the page lacks — a **"STAGE ATTRIBUTION" `.card`** of horizontal bar rows, one per funnel stage. Each row is a `.row`: a stage label (`.label`), a `Bar value={pct}` whose fill width = that stage's share of the **top** stage (Conversations = 100%), and a trailing `count · NN.N%` (`.mono`) where the % is conversion vs the **previous** stage (the existing `rate()` helper). Stages top→bottom: Conversations (`report.conversations`) → Recommendations (`report.recommendations`) → Booked (`report.byResponse.book`) → Consultations (`report.consultations`) → Revenue (`usd(report.bookedRevenueCents)` — value-only row, no bar). The booked stage uses the default crimson `.bar > i` fill. Keep the existing KPI `Stat` row above and the byTrigger/byResponse/byStatus breakdowns below; pagehead eyebrow `M10.2 · PLATFORM ATTRIBUTION`, window `<Select>` right. New `funnel` i18n keys: `stageAttribution`, `stageConversations`, `stageRecommendations`, `stageBooked`, `stageConsultations`, `stageRevenue` (EN+VI). **This pattern — pagehead → stat row → labeled card → breakdowns — is the model every other M19 task follows.**
>
> **Screen index** (full per-screen deltas live in each `PRD-TRACKING.md` M19.x.y item; screenshots in `requirements/design/screenshots/pages/`):
> - **Batch 1 · User app:** 01-history, 02-my-knowledge (=M18.3.2), 03-account.
> - **Batch 2 · Knowledge & experts:** 04-knowledge-detail, 05-knowledge-drafts, 06-knowledge-draft-detail, 07-experts, 08-expert-detail, 09-voice-profile-detail (=M13.5).
> - **Batch 3 · Concierge & quality:** 10-concierge-settings, 11-concierge-analytics, 12-answers, 13-failed-queries, 14-validation.
> - **Batch 4 · Growth & revenue:** 18-analytics, 19-funnel, 17-conversions, 15-recommendation-rules, 16-reconcile, 25-retention.
> - **Batch 5 · Platform:** 20-users, 21-user-detail, 22-access-control (=M14.4.3), 23-audit, 24-settings (=M17.5).
