...
- [x] M19.5.1 users (`apps/admin/app/users/page.tsx`, screenshot 20) — render "Manage" as a `Button` (not a `.navitem` link); render the premium plan as a colored `.badge` (not plain text); `.lede` intro. Keep the role `Select` + search `Input` + Apply + EMAIL / NAME / ROLE / PLAN / JOINED / Manage `Table`. Client `listUsers`, `AdminUserSummaryDto`. No new keys.
- [x] M19.5.2 user-detail (`apps/admin/app/users/[id]/page.tsx`, screenshot 21) — add the "← Back to users" eyebrow link (currently missing); confirm the 3 `.stat` cards (conversations / uploads / consultations) + Subscription / Role / FairUse / Deletion `.card` sections. Client `getUser`, `AdminUserDetailDto`. New `users` key: `detail.back`.
- [x] M19.5.3 access-control (`apps/admin/app/access-control/page.tsx`, screenshot 22) — **(verify+polish, M14.4.3)**: wrap the add-form row in a `.card .card-pad` panel; bold the "admin" / "expert" words in the intro (markup). Keep the email + role + Add form + `Table`. Client `listAllowedEmails` / `addAllowedEmail` / `updateAllowedEmail` / `removeAllowedEmail`, `AllowedEmailDto`. No new keys.
- [x] M19.5.4 audit (`apps/admin/app/audit/page.tsx`, screenshot 23) — align copy to the mockup (eyebrow "EVERY PRIVILEGED ACTION, RECORDED", subtitle "Who did what, when. Immutable — for compliance and incident review."); keep the WHEN / ACTOR / ACTION (`.badge-ink`) / TARGET / DETAIL `Table` + Load-more. Client `getAuditLogs`, `AdminAuditLogDto`. Update `audit` `eyebrow`/`subtitle`.
- [x] M19.5.5 settings (`apps/admin/app/settings/page.tsx`, screenshot 24) — **(verify+polish, M17.5)**: add a primary Save in the `.pagehead` (top-right) mirroring the existing bottom one; wrap the fields in a `.card .card-pad`; render the score-floor "GUIDE" as a `.chip` by the label. Keep the temp / model / scoreFloor / embedding `Field`s. Client `getAppSettings` / `updateAppSettings`, `AppSettingsDto`. Optional `settings` key: `scoreFloorGuide`.
