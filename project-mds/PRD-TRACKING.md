...
#### M18 — Uploaded document management ("My Knowledge") — PLANNED (§"M18 — Uploaded document management")
...
- [ ] M19.2.1 knowledge-detail (`apps/admin/app/knowledge/[id]/page.tsx`, screenshot 04) — **(verify+polish)**: confirm `.pagehead` back-eyebrow + `.h1` + `.muted .mono` scope/lang/versions + status `Badge`, and the versions `Table` (VERSION / STATUS / CHUNKS / CHANGE SUMMARY / CREATED / ACTIONS) with `DRAFT`/`LIVE` chips + `publishStatusTone` `Badge` tones render per screenshot. Client `getDocument` / `versionAction`, `KnowledgeDocumentDetailDto`. No new keys.
- [ ] M19.2.2 knowledge-drafts (`apps/admin/app/knowledge-drafts/page.tsx`, screenshot 05) — add a `.lede` subtitle under the `.h1`; keep the existing TITLE / STATUS / LANG / FROM CHAT / UPDATED `Table` (title `Link`, status `Badge`). **Omit the illustrative "· N asks" count** (no DTO field). Client `listDrafts`, `KnowledgeDraftSummaryDto`. New `knowledgeDrafts` key: `lede`.
- [ ] M19.2.3 knowledge-draft-detail (`apps/admin/app/knowledge-drafts/[id]/page.tsx`, screenshot 06) — add an info-callout `.card`/`.panel` (ⓘ) above the Title field; keep Title `Input` + Content `Textarea` + action `Button`s + back-eyebrow + status `Badge`. **Omit the "N asks" count + "View source queries" link** (no DTO backing) — render the static note only. Client `getDraft` / `updateDraft` / `draftAction`, `KnowledgeDraftDto`. New `knowledgeDrafts` key: `autoDraftedNote`.
- [ ] M19.2.4 experts (`apps/admin/app/experts/page.tsx`, screenshot 07) — move the "New expert" `Button` into `.pagehead` (right-aligned); add a leading `.avatar` (`avatarInitials`/`avatarTone`) cell before Name in the `Table`; render Manage as a `Button` (not a bare link); `.lede` intro. Client `listExperts` / `createExpert`, `AdminExpertSummaryDto`. No new keys.
