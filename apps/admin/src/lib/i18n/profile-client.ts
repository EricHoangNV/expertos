 ...
 * as a build arg (the value is public — it only identifies the endpoint). Mirrors `admin-client`.
 */
...
/**
 * Reads the acting user's persisted locale (`GET /me`, M13.3). Used to seed the locale on a fresh
 * device where localStorage has no cached preference yet. Admin/expert users are ordinary users in
 * the DB, so `/me` carries their locale just like the consumer app.
 */
...
/**
 * Persists the chosen locale onto the user profile (`PATCH /me/locale`, M13.3) so the preference
