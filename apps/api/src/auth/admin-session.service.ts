...
import type { AdminSessionDto, PortalRole } from "@expertos/shared";
 ...
 * source of truth for portal roles). A non-whitelisted email — or one whitelisted with only the
 * `user` role (a consumer-beta invite, see `AuthService.resolveUser`) — gets a 403 → the portal
 * renders its Access Denied screen.
 *
...
export class AdminSessionService {
  ...
  async resolve(user: AuthUser): Promise<AdminSessionDto> {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Only the two portal roles authorize the portal. A `user`-roled row is a legitimate
      // consumer-beta invite (it passes the beta gate in AuthService.resolveUser) but grants no
      // portal access.
      ...
      const role: PortalRole = entry.role;
