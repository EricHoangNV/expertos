...
/** localStorage key for the same-device locale preference (M13.1). */
const LOCALE_STORAGE_KEY = "expertos:locale";

interface LocaleContextValue {
  /** The active UI + answer locale. */
  locale: Locale;
...
/**
 * Owns the app-wide locale (M13.1). The locale drives both the UI language (via {@link useT}) and the
 * answer language passed to the chat API, unifying what used to be a chat-local `language` state.
 *
...
/** A translator bound to the active locale, optionally scoped to a `namespace` (M13.1). */
